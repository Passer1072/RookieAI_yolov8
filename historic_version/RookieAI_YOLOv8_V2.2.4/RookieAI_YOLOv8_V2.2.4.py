...
def create_gui_tkinter():  # 软件主题GUI界面
    ...
    version_number_text1.grid(row=3, column=0, padx=(30, 0), pady=(150, 0))
    # 版本号显示1
    version_number1 = ctk.CTkLabel(setting_frame, text="V2.2.4", width=30)
    version_number1.bind("<Button-1>", command=open_web)
    version_number1.grid(row=3, column=0, padx=(140, 0), pady=(150, 0))
    # 版本号显示2
    version_number_text2 = ctk.CTkLabel(setting_frame, text="最新版本:", width=30)
    version_number_text2.bind("<Button-1>", command=open_web)
    version_number_text2.grid(row=4, column=0, padx=(30, 0), pady=(0, 0))
    # 版本号显示2
    version_number2 = ctk.CTkLabel(setting_frame, text=":(", width=30)
    version_number2.bind("<Button-1>", command=open_web)
    version_number2.grid(row=4, column=0, padx=(140, 0), pady=(0, 0))
...
def load_settings():  # 加载主程序参数设置
    global model_file, test_window_frame
    ...
    try:
        ...
        prediction_factor_scale.set(settings.get('prediction_factor'))  # 使用适当的默认值来替换default_value
...
def calculate_distances(
    ...
    if minBox is not None:
        ...
        centerx = center_text_x - cWidth
        centery = center_text_y - cHeight - aimOffset_
