...
def create_gui_tkinter():  
    ...
    LookSpeed_frame.grid(row=6, column=0, sticky='w', pady=2)
    ...
    closest_mouse_dist_frame.grid(row=7, column=0, sticky='w', pady=2) 
    ...
    frame = ctk.CTkLabel(tab_view.tab("Basic"))
    frame.grid(row=8, column=0, sticky='w', pady=5) 
    ...
    message_text_frame.grid(row=9, column=0, sticky='w', pady=2)  
...
def update_values(*args):
    ...
    # aimOffset_Magnification_x = aimOffset_Magnification_x_scale.get()
...
def calculate_distances(
    ...
    if draw_center:
        ...
        if segmented_aiming_switch: 
            ...
            cv2.circle(frame_, (int(cWidth), int(cHeight)), int(stage1_scope), (0, 255, 255), 1)
            cv2.circle(frame_, (int(cWidth), int(cHeight)), radius=3, color=(0, 0, 255), thickness=-1)
        else:
            cv2.circle(frame_, (int(cWidth), int(cHeight)), int(closest_mouse_dist), (0, 255, 0), 1)
            cv2.circle(frame_, (int(cWidth), int(cHeight)), radius=3, color=(0, 0, 255), thickness=-1)
    ...
    for box in boxes:
        ...
        cv2.putText(frame_, f'dist: {dist}', location, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
    if minBox is not None:
        min_x1, min_y1, min_x2, min_y2 = minBox

        cv2.rectangle(frame_, (int(minBox[0]), int(minBox[1])), (int(minBox[2]), int(minBox[3])), (255, 255, 255), 1)
        ...
        if method_of_prediction == "Disable prediction":
            print("已Disable prediction")
            centerx *= lockSpeed
            centery *= lockSpeed
            pass

        elif method_of_prediction == "Rate prediction":
            print("Rate prediction is enabled")
