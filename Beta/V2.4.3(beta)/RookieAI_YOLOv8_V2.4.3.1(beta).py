...
recoil_single_distance = 5  # 每次向下移动距离（像素）
...
def load_settings():  # 加载主程序参数设置
    global model_file, test_window_frame, screenshot_mode, crawl_information, DXcam_screenshot, dxcam_maxFPS, \
        loaded_successfully, stage1_scope, stage1_intensity, stage2_scope, stage2_intensity, segmented_aiming_switch, \
        recoil_interval, recoil_single_distance, recoil_switch
    ...
    try:
        ...
        recoil_single_distance = settings.get("recoil_single_distance", 5)  # 每次向下移动距离（像素）
...
def recoil():  # 反后坐力
    ...
    # 判断反冲是否在前0.5秒内
    if current_time - recoil_start_time < 0.35:
        single_distance = boosted_distance  # 在前0.5秒内，使用增强的距离（力度较大）
    else:
        single_distance = standard_distance  # 在0.5秒之后，使用标准的距离（力度较小）
