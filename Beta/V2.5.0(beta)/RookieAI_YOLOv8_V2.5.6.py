    ...
    if minBox is not None:
        ...
        # 第三种：shift+按下触发
            ...
            if aimbot and ((lockKey_pressed and shift_pressed) or (mouse_Side_Button_Witch and xbutton2_pressed)):
                match mouse_control:
                    case '飞易来USB':
                        if offset_dist < 15 and coordinate_movement_switch:  # 如果距离小于15则绝对移动到坐标(一步到位)
                            dll.M_MoveTo2_D(ctypes.c_uint64(hdl), int(target_screen_x), int(target_screen_y))
                        else:
                            dll.M_MoveR2(ctypes.c_uint64(hdl), int(centerx), int(centery))
                    case 'win32':
                        win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, int(centerx),
                ...
                try:
                    ...
                    if next_points is not None and status is not None:
                        ...
                        if len(good_new) > 0:
                            ...
                            # 计算出现次数最多的方向
                            direction = max(set(direction_cache), key=direction_cache.count)
                            # 打印出现次数最多的方向
