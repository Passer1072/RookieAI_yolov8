...
def track_box_id(centerx, centery, box_width, box_height, monitor):  # 跟踪框ID
    ...
    min_range = 90
    max_range = 100
    identification_range = min_range + (max_range - min_range) * (box_area / (monitor["width"] * monitor["height"]))
...
def calculate_distances(
    ...
    global boxes, cWidth, cHeight, extra_offset_x, extra_offset_y, last_offset_time, aimOffset, enable_random_offset\
        , last_recoil_time, counter_id, buffer_tracks, last_updated, locked_id, slow_aim_speed, stop_aim\
        , stop_aim_variety, stop_aim_variety_state, previous_centerx, previous_centery
    ...
    if minBox is not None:
        ...
        if method_of_prediction == "禁用预测":
            ...
            # 建议值和注释
            # prediction_factor: 用于确定倍率预测模式下的瞄准提前量。
            # 建议值: 0.2 - 0.5（根据目标速度适当调整）
            # 较高的值会导致更大的提前瞄准量，适用于快速移动的目标。

            # threshold: 检测目标是否“停止”的速度阈值。
            # 建议值: 1.0 - 3.0（像素单位）
            # 如果目标的移动速度低于该值，则认为目标停止或接近停止，从而减慢瞄准速度。
            threshold = 2.0

            # slowDownFactor: 目标停止时的减速因子。
            # 建议值: 0.1 - 0.5
            # 这个值越小，停止时瞄准速度减得越多，从而减少超调的可能性。
            slowDownFactor = 0.3

            # normalSpeed: 目标移动时的正常锁定速度。
            # 建议值: 1.0 - 2.0
            # 这是倍率预测模式下的标准锁定速度。
            normalSpeed = 1.3

            # 预设的阈值和参数
            reverse_threshold = 5  # 短时间内反向移动的阈值
            smoothing_factor = 0.8  # 平滑因子，接近1表示更平滑的跟踪
            ...
            # 预测新的瞄准点
            centerx_predicted = centerx + (centerx_extra if centerx > 0 else -centerx_extra)
            centery_predicted = centery + (centery_extra if centery > 0 else -centery_extra)

            # 检测短时间内反向移动，并进行过滤
            if abs(centerx_predicted - previous_centerx) < reverse_threshold:
                # 忽略反向移动，保持原方向
                centerx_predicted = previous_centerx
            if abs(centery_predicted - previous_centery) < reverse_threshold:
                # 忽略反向移动，保持原方向
                centery_predicted = previous_centery
            ...
            centery = (previous_centery * (1 - smoothing_factor)) + (centery_predicted * smoothing_factor)
        ...
        # 动态计算停止自瞄范围（圆形）
        # stop_aim_radius = minBox_width * stop_aim
        # cv2.circle(frame_, (int(offset_centerx), int(offset_centery)), int(round(stop_aim_radius)), (255, 0, 0), 2)

        # # 动态计算停止自瞄范围（长方形）
        # top_ratio = 0.1  # 上半部分高度的比例 0.15
        # bottom_ratio = 0.45  # 下半部分高度的比例 0.5
        #
        # if stop_aim_variety_state:
        #     stop_aim_variety = stop_aim  # 回到原始值
        # stop_aim_width = minBox_width * stop_aim_variety  # 停止自瞄范围的宽度
        # total_stop_aim_height = minBox_height * (top_ratio + bottom_ratio)  # 停止自瞄范围的总高度
        #
        # # 确保停止自瞄范围的总高度不超过目标框的高度
        # if total_stop_aim_height > minBox_height:
        #     top_ratio = minBox_height * top_ratio / total_stop_aim_height
        #     bottom_ratio = minBox_height * bottom_ratio / total_stop_aim_height
        #
        # stop_aim_height_top = minBox_height * top_ratio  # 停止自瞄范围的上半部分高度
        # stop_aim_height_bottom = minBox_height * bottom_ratio  # 停止自瞄范围的下半部分高度
        #
        # stop_aim_left = offset_centerx - stop_aim_width / 2
        # stop_aim_right = offset_centerx + stop_aim_width / 2
        # stop_aim_top = offset_centery - stop_aim_height_top
        # stop_aim_bottom = offset_centery + stop_aim_height_bottom
        #
        # # 确保停止自瞄范围的上下边界不超出目标框的上下边界
        # if stop_aim_top < min_y1:
        #     stop_aim_top = min_y1
        # if stop_aim_bottom > min_y2:
        #     stop_aim_bottom = min_y2
        #
        # cv2.rectangle(frame_, (int(stop_aim_left), int(stop_aim_top)), (int(stop_aim_right), int(stop_aim_bottom)),
        #               (255, 0, 0), 2)  # 绘制停止自瞄范围长方形
        #
        # # 计算光标到目标中心的距离
        # target_distance = sqrt(centerx ** 2 + centery ** 2)
        #
        # # 检查光标是否在目标框内进行强锁
        # # 按选定的比例扩大边界框尺寸
        # expanded_min_x1 = min_x1 - ((min_x2 - min_x1) * expansion_ratio / 2)
        # expanded_min_x2 = min_x2 + ((min_x2 - min_x1) * expansion_ratio / 2)
        # expanded_min_y1 = min_y1 - ((min_y2 - min_y1) * expansion_ratio / 2)
        # expanded_min_y2 = min_y2 + ((min_y2 - min_y1) * expansion_ratio / 2)
        #
        # if draw_center:
        #     cv2.rectangle(frame_, (int(expanded_min_x1), int(expanded_min_y1)), (int(expanded_min_x2),
        #                                                                          int(expanded_min_y2)), (255, 255, 0),
        #                   1)
        #
        # # 将鼠标光标移动到检测到的框的中心
        # # 判断是否减慢自瞄速度
        # if stop_aim_left <= cWidth <= stop_aim_right and stop_aim_top <= cHeight <= stop_aim_bottom:
        #     slow_aim_speed = lockSpeed * 0.1  # 在停止自瞄范围内减慢自瞄速度，假设减慢为原速度的%
        # else:
        #     # 检查光标是否在扩展边界框内
        #     if expanded_min_x1 <= cWidth <= expanded_min_x2 and expanded_min_y1 <= cHeight <= expanded_min_y2:
        #         slow_aim_speed = stage1_intensity
        #         pass
        ...
        if triggerType == "切换":
            if aimbot and (win32api.GetKeyState(lockKey) or (mouse_Side_Button_Witch and xbutton2_pressed)):
                match mouse_control:
                    case '飞易来USB':
                        dll.M_MoveR2(ctypes.c_uint64(hdl), int(centerx * slow_aim_speed), int(centery * slow_aim_speed))
                    case 'win32':
                        win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, int(centerx * slow_aim_speed),
                                             int(centery * slow_aim_speed), 0, 0)
                    case 'mouse':
                        mouse.move(int(centerx * slow_aim_speed), int(centery * slow_aim_speed), False)
                    case 'Logitech':
                        LG_driver.move_R(int(centerx * slow_aim_speed), int(centery * slow_aim_speed))
                if offset_dist <= 30:
                    AFe.start()
                else:
                    AFe.stop()
        ...
        # 第二种：按下触发
            ...
            if aimbot and (lockKey_pressed or (mouse_Side_Button_Witch and xbutton2_pressed)):
                ...
                stop_aim_variety = stop_aim
                stop_aim_variety_state = True  # 需要回到默认值
                AFe.stop()
        ...
        # 第三种：shift+按下触发
            ...
            if aimbot and ((lockKey_pressed and shift_pressed) or (mouse_Side_Button_Witch and xbutton2_pressed)):
                match mouse_control:
                    case '飞易来USB':
                        dll.M_MoveR2(ctypes.c_uint64(hdl), int(centerx * slow_aim_speed), int(centery * slow_aim_speed))
                        if stop_aim_variety >= stop_aim_target:
                            stop_aim_variety -= 0.08  # 宽度逐渐变窄
                            if stop_aim_variety < 0:  # 如果小于0，直接设为0
                                stop_aim_variety = 0.0
                        stop_aim_variety_state = False  # 不需要回到默认值
                        print('stop_aim_variety:', stop_aim_variety)
                    case 'win32':
                        win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, int(centerx * slow_aim_speed),
                                             int(centery * slow_aim_speed), 0, 0)
                    case 'mouse':
