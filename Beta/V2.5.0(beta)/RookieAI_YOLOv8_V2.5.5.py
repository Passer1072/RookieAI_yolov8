...
def process_aiming(centerx_predicted, centery_predicted, previous_centerx, previous_centery,  # 鼠标移动平滑
    ...
    # 指数平滑
    centerx_smoothed = (previous_centerx * (1 - smoothing_factor)) + (centerx_predicted * smoothing_factor)
    centery_smoothed = (previous_centery * (1 - smoothing_factor)) + (centery_predicted * smoothing_factor)
...
def create_gui_tkinter():  # 软件主题GUI界面
    global aimbot_var, lockSpeed_scale, triggerType_var, arduinoMode_var, lockKey_var, confidence_scale \
        ...
        , tolerance_variable, tolerance_scale, ignore_colors_entry, ignore_colors_variable, automatic_Trigger_var \
        , mouse_movement_smoothing_switch, smooth_aiming_var, predict_model_var
    ...
    predict_model_frame.grid(row=10, column=0, sticky='w', pady=5)  # frame在root窗口中的位置
    # 创建一个标签文本并将其插入frame中
    ...
    # 创建OptionMenu并使用lockKey_var和options
    predict_model_menu = ctk.CTkOptionMenu(predict_model_frame, variable=predict_model_var, values=options, command=update_values)
    ...
    smooth_aiming_check = ctk.CTkCheckBox(smooth_aiming_frame, text='平滑瞄准', variable=smooth_aiming_var, command=update_values)
    smooth_aiming_check.grid(row=0, column=0)  # 使用grid布局并靠左对齐

    # 平滑瞄准参数配置按钮
    smooth_aiming_set_button = ctk.CTkButton(smooth_aiming_frame, width=50, image=None, command=show_recoil_window, text="配置参数")
    smooth_aiming_set_button.grid(row=0, column=2, sticky="n", padx=(130, 0), pady=(0, 0))

    # 2创建一个Frame来包含OptionMenu和其左边的Label-
    message_text_frame = ctk.CTkFrame(tab_view.tab("基础设置"))
    message_text_frame.grid(row=14, column=0, sticky='w', pady=2)  # 使用grid布局并靠左对齐
    # 创建Label
...
def update_values(*args):
    global aimbot, lockSpeed, triggerType, arduinoMode, lockKey, lockKey_var, confidence, closest_mouse_dist \
        ...
        , ignore_colors, automatic_Trigger, mouse_movement_smoothing_switch, predict_model
...
def load_prefix_variables():  # 加载前置参数
    ...
    try:
        ...
        segmented_aiming_switch = Opt.get("segmented_aiming_switch", False)  # 加载分段瞄准开关
...
def load_settings():  # 加载主程序参数设置
    ...
    prediction_factor_scale.set(Opt.get('prediction_factor', 0.5))  # 使用适当的默认值来替换default_value
...
def predict_aim_position(box_centerx, minBox_width, min_x1, min_x2, previous_centerx, direction, progress=0.6):
    ...
    if direction == "左":
        ...
        if min_x1 + right_offset_percentage * minBox_width <= target_centerx <= min_x2:
            box_centerx = smooth_transition(previous_centerx, target_centerx, progress)
        else:
        ...
        # print("无预测")
        box_centerx = smooth_transition(previous_centerx, box_centerx, progress)
...
def calculate_distances(
    ...
    if minBox is not None:
        ...
        cv2.rectangle(frame_, (int(minBox[0]), int(minBox[1])), (int(minBox[2]), int(minBox[3])), (0, 255, 0), 2)  # 最近的框为绿色
        ...
        if predict_model == "自动预测":
            box_centerx = predict_aim_position(box_centerx, minBox_width, min_x1, min_x2, previous_centerx, direction)
        ...
        if method_of_prediction == "禁用加速":
            ...
            # 预测新的瞄准点(预测后位置)
            centerx_predicted = (centerx + (centerx_extra if centerx > 0 else -centerx_extra)) * lockSpeed
            centery_predicted = (centery + (centery_extra if centery > 0 else -centery_extra)) * lockSpeed
...
def sparse_optical_flow_inference(frame, prev_gray, prev_points, screen_width, screen_height, optical_flow_frame_counter):  # 稀疏流光推理函数
    ...
    # 仅每8帧进行一次稀疏光流推理
    if optical_flow_frame_counter % 5 == 0:
        if prev_gray is not None:
            # 检查是否需要重新检测特征点
            if prev_points is None or len(prev_points) < 10:
                prev_points = cv2.goodFeaturesToTrack(prev_gray, maxCorners=300, qualityLevel=0.2, minDistance=5, blockSize=7)
            ...
            if prev_points is not None and len(prev_points) > 0:
                ...
                # 定义中心区域的大小（相对于图像尺寸）
                center_margin_x = 0.3  # X轴方向中心区域的宽度比例
                center_margin_y = 0.3  # Y轴方向中心区域的高度比例
                ...
                try:
                    ...
                    if next_points is None or status is None or len(next_points) < 10:
                        # 重新检测特征点
                        prev_points = cv2.goodFeaturesToTrack(gray, maxCorners=300, qualityLevel=0.2, minDistance=5, blockSize=7)
                        next_points, status, err = cv2.calcOpticalFlowPyrLK(prev_gray, gray, prev_points, None)
