...
ignore_colors = None  # Default value, list of BGR tuples
...
def create_gui_tkinter():  # 软件主题GUI界面
    ...
    # 实例化 CTkTabview 对象
    tab_view = ctk.CTkTabview(root, width=320, height=550)
    # 创建选项卡
    tab_view.add("基础设置")
    tab_view.add("高级设置")
    tab_view.add("其他设置")
...
def update_values(*args):
    ...
    try:
        ...
        ignore_colors_variable.set(str(ignore_colors))  # 设置默认值
...
def save_settings():  # 保存设置
    ...
    new_settings = {
        ...
        'time_interval': float(offset_time_entry.get()),  # 随机瞄准切换间隔
        'recoil_switch': recoil_var.get(),  # 辅助压枪开关
        'tolerance': tolerance_scale.get()  # 颜色忽略误差值
    }
...
def load_settings():  # 加载主程序参数设置
    ...
    try:
        ...
        recoil_standard_distance_entry.insert(0, str(settings.get("recoil_standard_distance", 1)))  # 二阶段单次距离
        recoil_transition_time_entry.insert(0, str(settings.get("recoil_transition_time", 0.2)))  # 缓冲时间
        tolerance_scale.set(settings.get("tolerance", 50))   # 颜色忽略
        ignore_colors_entry.insert(0, str(settings.get("ignore_colors", [[0, 0, 0]])))
