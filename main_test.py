...
def create_gui_tkinter():
    global aimbot_var, lockSpeed_scale, triggerType_var, arduinoMode_var, lockKey_var, confidence_scale\
        , closest_mouse_dist_scale, screen_width_scale, screen_height_scale, root
    ...
    # 文档显示部分（测试）
    # # 创建新的Frame部件以容纳显示.txt文件内容的Text部件
    # txt_frame = tk.Frame(root)
    # txt_frame.grid(row=10, column=0)  # txt_frame在root窗口中的位置
    #
    # # 创建弹簧挤压避免Text部件填满窗口
    # txt_frame.grid_rowconfigure(0, weight=1)
    # txt_frame.grid_columnconfigure(0, weight=1)
    #
    # # 创建Text部件，并将其插入到txt_frame中
    # txt_display = tk.Text(txt_frame, wrap='word')  # 'word'参数意味着Text部件将在单词边界自动换行
    # txt_display.grid(row=0, column=0, sticky='nsew')  # Text部件在txt_frame中的位置
    #
    # # 创建一个滚动条，并将其插入到txt_frame中，使其与txt_Display对齐
    # scrollbar = tk.Scrollbar(txt_frame, command=txt_display.yview)
    # scrollbar.grid(row=0, column=1, sticky='ns')
    # txt_display['yscrollcommand'] = scrollbar.set  # 当Text部件内容滚动时
    #
    # # 打开并读取文件的内容
    # with open('键值.txt', 'r', encoding='utf-8') as file:
...
def create_gui_PySimpleGUI():
    global root, aimbot_var, lockSpeed_scale, triggerType_var, arduinoMode_var, lockKey_var, confidence_scale, \
        closest_mouse_dist_scale, screen_width_scale, screen_height_scale

    # Define layout
    layout = [[sg.Text('RookieAI-yolov8版本', font='Helvetica 24')],
              [sg.Checkbox('Aimbot(自瞄开关)', default=False, key='aimbot')],
              [sg.Checkbox('Arduino Mode(待开发)', default=False, key='arduino')],
              [sg.OptionMenu(values=["按下", "按下", "切换", "shift+按下"], default_value="按下", key='trigger')],
              [sg.Text('当前热键为: '), sg.OptionMenu(values=['右键', '左键', '右键', '下侧键'], key='lockKey')],
              [sg.Slider(range=(0.00, 1.00), resolution=0.01, default_value=0.5, orientation='h', size=(15, 20),
                         key='lockSpeed')],
              [sg.Slider(range=(0.00, 1.00), resolution=0.01, default_value=0.5, orientation='h', size=(15, 20),
                         key='confidence')],
              [sg.Slider(range=(0, 300), resolution=1, default_value=150, orientation='h', size=(15, 20),
                         key='mouse_dist')],
              [sg.Slider(range=(100, 2000), resolution=10, default_value=500, orientation='h', size=(15, 20),
                         key='screen_width')],
              [sg.Slider(range=(100, 2000), resolution=10, default_value=500, orientation='h', size=(15, 20),
                         key='screen_height')],
              [sg.Button('保存设置'), sg.Button('加载设置'),
               sg.Button('关闭', button_color=('white', 'red'), key='-CLOSE-')]
              ]

    # Create window
    root = sg.Window('My new window', layout)
...
def update_values(*args):
    global aimbot, lockSpeed, triggerType, arduinoMode, lockKey, lockKey_var, confidence, closest_mouse_dist\
        , closest_mouse_dist_scale, screen_width, screen_height
...
def main():
    global run_threads  # 声明全局变量
    run_threads = True  # 初始化为True
    # 启动 GUI
    create_gui_tkinter()
